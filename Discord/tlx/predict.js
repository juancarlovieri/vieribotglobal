...
module.exports = {
  calc: async function (bot, msg, usr, contest) {
    ...
    var coba = request(
      'GET',
      'https://uriel.tlx.toki.id/api/v2/contest-web/slug/' +
        contest +
        '/with-config'
    );
    ...
    request(
      ...
      (error, response, body) => {
        ...
        var uti = new Map(
          Object.entries(JSON.parse(response.body).profilesMap)
        );
        var uid = -1;
        ...
        delta /= n;
        var debt =
          getRating(uid, id, uti).hiddenRating -
          getRating(uid, id, uti).publicRating;
        // var newRating = 1800;
