...
function command(args, msg) {
  ...
  switch (args[0]) {
    case '^help':
      ...
      str +=
        '**^bigmoji <emoji>** - converts emoji to a big image (only works on server emoji)\n';
    ...
    case '^idler':
      ...
      if (
        ...
        msg.author.id != '758191365065670687'
      )
        return;
    ...
    case '^bigmoji':
      ...
      msg.channel.send(
        'https://cdn.discordapp.com/emojis/' +
          args[1].substr(indx, args[1].length - indx - 1) +
          '.png'
      );
      break;
    case '^predict':
      ...
      os.cpuUsage(function (v) {
        msg.channel.send(
          '**CPU usage:** ' +
            ...
            os.cpuCount() +
            '\n**RAM Usage**: ' +
            Math.round((os.totalmem() - os.freemem()) * 100) / 100 +
            ' mb /' +
            Math.round(os.totalmem() * 100) / 100 +
            ' mb'
        );
      });
    ...
    case '^send':
      ...
      msg.channel.send('discord part 1:', {
        files: [
          'datas/changeHandle.json',
          './ongoing.json',
          './ongoingAtcoder.json',
          './ongoingTeam.json',
          './teamChallenge.json',
      ...
      msg.channel.send('discord part 2: ', {
        files: [
          './bot.js',
          '../lagrange_rank.json',
          './links.json',
      ...
      msg.channel.send('discord part 3: ', {
        files: ['./gambleLocks.json', 'gambles.json', 'balance.json'],
      });
      msg.channel.send('telegram:', {
        files: [
          '../Telegram/handles.json',
          '../Telegram/ongoing.json',
          '../Telegram/problems.json',
        ],
      });
      break;
    case '^translate':
...
function play() {
  ...
  channel
    ...
    .then((connection) => {
      ...
      connection
        .play(ytdl('https://youtu.be/hGlyFc79BUE'))
        .on('finish', () => {
          play();
        })
