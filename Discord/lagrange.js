...
async function newGlobal(args, msg) {
  ...
  if (glob != -1) {
    msg.channel.send(
      'there is an ongoing global duel, integer is: **' + glob + '**'
    );
    return;
  }
  var l = args[1],
...
async function newGlobalRepeat(msg) {
  if (ranked) msg.channel.send('repeating, previous range was: ranked');
    ...
    msg.channel.send(
      'repeating, previous range was: ' + prevL + ' and ' + prevR
    );
  var l = prevL,
    r = prevR;
  startTime = Date.now();
...
function reveal(msg, id) {
  ...
  if (canceler < 3 && id != prevSolver) {
    msg.channel.send('not enough people to cancel');
    return;
  }
  cancelList.clear();
  console.log(cancelList);
...
module.exports = {
    ...
    switch (args[1]) {
      ...
      case 'ch':
        ...
        if (challenge.has(opp)) {
          msg.channel.send(
            'someone is challenging the one you are challenging'
          );
          return;
        }
        if (ongoing.has(opp)) {
        ...
        msg.channel.send(
          '<@' +
            ...
            '>, <@' +
            msg.author.id +
            '> is challenging you on a lagrange duel!'
        );
        break;
      case 'reveal':
      ...
      case 'fs':
        if (
          ...
          msg.author.id != '545083775906873364'
        ) {
          return;
        }
        glob = '-1';
  ...
  isAns: async function (bot, msg) {
    ...
    var troll = 1;
    if (msg.author.id != '455184547840262144' && glob == '0' && res != '1')
      troll = 0;
    if (res == glob && troll) {
      glob = '-1';
  ...
  repeat: function (msg, emoji, user) {
    ...
    lastId = -1;
    if (glob != -1) {
      msg.channel.send('there is an ongoing');
      return;
    }
    newGlobalRepeat(msg);
  },
