  ...
  duel: function (bot, msg) {
    ...
    switch (args[1]) {
      ...
      case 'regis':
        ...
        msg.channel.send(
          '<@' +
            msg.author.id +
            '> submit ' +
            crypto.createHash('sha256').update(msg.author.id).digest('hex') +
            ' to any problem within 1 hour'
        );
        break;
      case 'regisdone':
        ...
        for (var i = 0; i < submissions.length; i++) {
          if (
            submissions[i].epoch_second * 1000 < temp.time ||
            submissions[i].epoch_second * 1000 > temp.time + 3600000
          )
            continue;
          var body = request(
            'GET',
            'https://atcoder.jp/contests/' +
              submissions[i].contest_id +
              '/submissions/' +
              submissions[i].id
          ).getBody();
      ...
      case 'problem':
        ...
        var request = require('sync-request');
        var problems = JSON.parse(
          request(
            'GET',
            'https://kenkoooo.com/atcoder/resources/contest-problem.json'
          ).getBody()
        );
        var submission = JSON.parse(
          request(
            'GET',
            'https://kenkoooo.com/atcoder/atcoder-api/results?user=' +
              handles.get(msg.author.id)
          ).getBody()
        );
        ...
        for (var i = 0; i < problems.length; i++) {
          if (problems[i].contest_id.substr(0, args[2].length) != args[2])
            continue;
          if (
            args[3].toLowerCase() == problems[i].problem_id[7] ||
            num.toString() == problems[i].problem_id[7]
          ) {
            var can = 1;
        ...
        msg.channel.send(
          'problem for <@' +
            msg.author.id +
            '>: https://atcoder.jp/contests/' +
            problems[indx].contest_id +
            '/tasks/' +
            problems[indx].problem_id
        );
        break;
      case 'challenge':
        if (args.length != 4) {
        ...
        if (challenge.has(msg.author.id)) {
          msg.channel.send(
            'someone is challenging you or you are challenging someone'
          );
          return;
        }
        if (challenge.has(args[3].substr(3, args[3].length - 4))) {
          msg.channel.send(
            'yang lu challenge lagi di challenge ornag, males ngetik gw. paling ini message g bakal kekirim. ah ada typo lagi'
          );
          return;
        }
        var temp = {
      ...
      case 'accept':
        ...
        var submission2 = JSON.parse(
          request(
            'GET',
            'https://kenkoooo.com/atcoder/atcoder-api/results?user=' +
              handles.get(temp.opp)
          ).getBody()
        );
        var out =
          'problem for <@' + msg.author.id + '> and <@' + temp.opp + '> \n';
        for (var i = 0; i < 5; i++) {
          problems = shuffleArray(problems);
          var done = 0;
          for (var j = 0; j < problems.length; j++) {
            if (problems[j].problem_id[7] != need[i]) continue;
            if (problems[j].problem_id.substr(0, temp.type.length) != temp.type)
              continue;
            var can = 1;
            for (var k = 0; k < submission1.length; k++) {
              if (submission1[k].problem_id == problems[j].problem_id) {
                can = 0;
                break;
              }
            }
            for (var k = 0; k < submission2.length; k++) {
              if (submission2[k].problem_id == problems[j].problem_id) {
                can = 0;
                break;
              }
            }
      ...
      case 'update':
        ...
        for (var i = 0; i < temp.prob.length; i++) {
          for (var j = 0; j < submission1.length; j++) {
            if (
              submission1[j].result == 'AC' &&
              temp.prob[i].problem_id == submission1[j].problem_id
            ) {
          ...
          for (var j = 0; j < submission2.length; j++) {
            if (
              submission2[j].result == 'AC' &&
              temp.prob[i].problem_id == submission2[j].problem_id
            ) {
