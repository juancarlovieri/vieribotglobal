...
async function save_new(dbTitle, path, arr) {
  await col.updateOne({title: dbTitle}, {$set: {title: dbTitle, val: jsonContent}}, (err, res) => {
    if (err) {console.error(err); return;}
  });
  var jsonObj = Object.fromEntries(arr);
...
async function save(){
  ...
  await save_new("monitor", pathPerms, perms);
  await save_new("perms", pathPlayers, players);
}
...
async function init() {
  ...
  perms = read(pathPerms);
  players = read(pathPlayers);
  save();
}

init();

function pbEmbed(opt) {
  const dat = opt.dat;
  const rank = opt.rank;
  const cur = opt.cur;
  const time = opt.time;
  var embed = {
    color: "#0394fc",
    title: cur.user.username.toUpperCase() + " just achieved a new " + opt.gameMode + " personal best!",
    url: 'https://tetr.io/#r:' + cur.replayid,
    description: "**" + opt.score + "**",
    fields: [
      { name: 'Rank', value: rank, inline: true },
      { name: 'PPS', value: (dat.piecesplaced/time).toFixed(2), inline: true },
  ...
  if (opt.level) embed.fields.splice(4, 0, { name: 'Level', value: (dat.level).toFixed(0), inline: true });
  return embed;
}
    ...
    for (var temp of curm) {
      ...
      if (record.blitz.record != null) {
        ...
        if (val.blitz == null || newblitz > val.blitz) {
          ...
          const embed = pbEmbed({
            gameMode: "blitz",
            score: cur.endcontext.score.toFixed(0),
            time: 120,
            level: true,
            dat: dat,
            cur: cur,
      ...
      if (record["40l"].record != null) {
        ...
        if (val["40l"] == null || new40l < val["40l"]) {
          ...
          const embed = pbEmbed({
            gameMode: "40 lines",
            score: prettyMilliseconds(cur.endcontext.finalTime, {secondsDecimalDigits: 3}),
            time: (new40l/1000),
            level: false,
            dat: dat,
            cur: cur,
...
function lbEmbed(opt) {
  ...
  for (var i = 0; i < ans.length; ++i) {
    ...
    var spaces2 = " ";
    for (var j = 0; j < maxScoreChar - ans[i].score.length; ++j) spaces2 += " ";
    str += (i + 1).toFixed() + "." + spaces + ans[i].score + spaces2 + "| " + ans[i].user.username + '\n';
  }
  str += "```"
  const embed = {
    color: "#ebc334",
    title: opt.gameMode + " Leaderboard for " + country + (country.length == 2 ? (" :flag_" + country.toLowerCase() + ":") : ""),
    description: str,
    timestamp: new Date(),
    footer: {
      text: "By Vieri Corp.™ All Rights Reserved"
...
async function blitzLb(bot, msg, country) {
  ...
  for (var i = 0; i < records.length; ++i) {
    ...
    if (arr.includes(records[i].user._id)) {ans.push(records[i]); ans[ans.length - 1].score = ans[ans.length - 1].endcontext.score.toFixed();}
  }
  const embed = lbEmbed({
    gameMode: "Blitz",
    ans: ans,
...
async function fortyLinesLb(bot, msg, country) {
  ...
  for (var i = 0; i < records.length; ++i) {
    if (ans.length >= 50) break;
    if (arr.includes(records[i].user._id)) {ans.push(records[i]); ans[ans.length - 1].score = (ans[ans.length - 1].endcontext.finalTime / 1000).toFixed(3);}
  }
  const embed = lbEmbed({
    gameMode: "40l",
    ans: ans,
...
async function printGlobal(bot, msg, args) {
  if (args[2] == "blitz") {
    ...
    for (var i = 0; i < ans.length; ++i) ans[i].score = ans[i].endcontext.score.toFixed();
    const embed = lbEmbed({
      gameMode: "Blitz",
      ans: ans,
      country: "global"
    ...
    for (var i = 0; i < ans.length; ++i) ans[i].score = (ans[i].endcontext.finalTime / 1000).toFixed(3);
    const embed = lbEmbed({
      gameMode: "40l",
      ans: ans,
      country: "global"
...
async function printMonitored(bot, msg, args) {
  ...
  for (var i = 0; i < all.length; ++i) {
    if (args[2] == "blitz") all[i].score = all[i].score.toFixed();
    else if (args[2] == "40l") all[i].score = (all[i].score/1000).toFixed(3);
    all[i].user = {
...
async function new_monitor(args, msg, bot) {
  if (args.length != 3) {
