...
function win(winner, looser) {
  ...
  ratingW[ratingW.length] =
    ratingW.length == 0 ? 1500 + 50 : ratingW[ratingW.length - 1] + 50;
  ratingL[ratingL.length] =
    ratingL.length == 0 ? 1500 - 50 : ratingL[ratingL.length - 1] - 50;
...
function printratingHist(msg, bot) {
  ...
  var hasil = rating
    .get(map.get(args[2].substr(3, args[2].length - 4)))[0]
    .toString();
  for (
    var i = 1;
    i < rating.get(map.get(args[2].substr(3, args[2].length - 4))).length;
    i++
  ) {
    hasil +=
      ' - ' +
      rating.get(map.get(args[2].substr(3, args[2].length - 4)))[i].toString;
  }
...
function takeRating(user) {
  ...
  var ratings = request(
    'GET',
    'http://codeforces.com/api/user.rating?handle=' + user
  );
  ...
  duel: function (bot, msg) {
    ...
    switch (args[1]) {
      ...
      case 'regis':
        ...
        msg.channel.send(
          'submit ' +
            crypto.createHash('sha256').update(msg.author.id).digest('base64') +
            '  to any problem'
        );
        break;
      case 'regisdone':
              ...
              request2(
                ...
                function (error, response, body) {
                  var indx = body.indexOf(
                    crypto
                      .createHash('sha256')
                      .update(msg.author.id)
                      .digest('base64')
                  );
                  if (
                    indx < 0 ||
                    Date.now() - 3600000 >
                      submission.result[0].creationTimeSeconds * 1000
                  ) {
                    msg.channel.send(
                      'submit ' +
                        crypto
      ...
      case 'solved':
        ...
        const coba = request(
          'GET',
          'http://codeforces.com/api/user.rating?handle=' + args[2]
        );
        ...
        var submissions = request(
          'GET',
          'http://codeforces.com/api/user.status?handle=' +
            args[2] +
            '&from=1&count=1000000000&handle=' +
          ...
          for (var i = submission.length - 1; i >= 0; i--) {
            ...
            if (temp.y.length == 0 || temp.y[temp.y.length - 1] != d) {
              if (temp.y.length == 0) {
                ...
                temp.submitTime[temp.submitTime.length] =
                  submission[i].creationTimeSeconds;
                continue;
              }
              while (
                convertTime(
                  temp.submitTime[temp.submitTime.length - 1] + 86400
                ) != d
              ) {
        ...
        plotly.plot(data, graphOptions, function (err, mesg) {
          ...
          download(mesg.url + '.jpeg', 'display.png', function () {
            const embed = new Discord.MessageEmbed()
              .setTitle('Attachment')
              .setImage('attachment://display.png');
          ...
          function (error, response, body) {
            ...
            if (
              ...
              submission.problem.contestId ==
                problem.get(msg.author.id).contestId &&
              submission.problem.index == problem.get(msg.author.id).index
            ) {
              var res = [];
              var delta =
                10 +
                ((problem.get(msg.author.id).rating - 800) / 100) *
                  ((problem.get(msg.author.id).rating - 800) / 100);
              ...
              save();
              msg.channel.send(
                '<@' + msg.author.id + '> gained ' + delta + ' points'
              );
              return 1;
            } else {
              msg.channel.send(
                'solve this problem, <@' +
                  ...
                  problem.get(msg.author.id).contestId +
                  '/problem/' +
                  problem.get(msg.author.id).index
              );
            }
          }
        );
      ...
      case 'rating':
        ...
        msg.channel.send(
          getDuelRating(args[2].substr(3, args[2].length - 4)).toString()
        );
        break;
      case 'ratinggraph':
      ...
      case 'pointgraph':
        ...
        for (var i = 2; i < args.length; i++) {
          ...
          if (
            points.has(map.get(args[i].substr(3, args[i].length - 4))) == false
          ) {
            temp.y = [0];
          } else {
      ...
      case 'pointhist':
        if (args.length == 3) {
          if (map.has(args[2]) == false) {
            ...
            if (points.has(map.get(args[2])) == false) {
              ...
              for (var i = 1; i < points.get(map.get(args[2])).length; i++)
                hasil += ' - ' + points.get(map.get(args[2]))[i].toString();
        ...
        if (points.has(map.get(msg.author.id)) == false) {
          ...
          for (var i = 1; i < points.get(map.get(msg.author.id)).length; i++)
            hasil += ' - ' + points.get(map.get(msg.author.id))[i].toString();
          msg.channel.send(hasil);
        }
        break;
      case 'point':
          ...
          if (map.has(args[2]) == false) {
            ...
            if (points.has(map.get(args[2])) == false) {
              ...
              msg.channel.send(
                points
                  .get(map.get(args[2]))
                  [points.get(map.get(args[2])).length - 1].toString()
              );
            }
          }
          break;
        ...
        if (points.has(map.get(msg.author.id)) == false) {
          ...
          msg.channel.send(
            points
              .get(map.get(msg.author.id))
              [points.get(map.get(msg.author.id)).length - 1].toString()
          );
        }
        break;
      case 'complete':
        ...
        request(
          ...
          function (error, response, body) {
            ...
            if (
              submission.verdict == 'OK' &&
              submission.problem.contestId ==
                ongoing.get(msg.author.id)[1].contestId &&
              submission.problem.index == ongoing.get(msg.author.id)[1].index
            ) {
              ...
              msg.channel.send(
                '<@' +
                  msg.author.id +
                  '> has beaten <@' +
                  ongoing.get(msg.author.id)[0] +
                  '> in a ' +
                  ongoing.get(msg.author.id)[1].rating +
                  ' duel'
              );
              ...
              msg.channel.send(
                'solve this problem, <@' +
                  ...
                  ongoing.get(msg.author.id)[1].contestId +
                  '/problem/' +
                  ongoing.get(msg.author.id)[1].index
              );
            }
          }
        );
      ...
      case 'accept':
        ...
        for (var temp = 0; temp < problems.length; temp++) {
          ...
          var submissions = request2(
            'GET',
            'http://codeforces.com/api/contest.status?contestId=' +
              problems[temp].contestId +
              '&from=1&count=1000000&handle=' +
              map.get(msg.author.id)
          );
          var submission = JSON.parse(submissions.getBody()).result;
          var can = 1;
          ...
          submissions = request2(
            'GET',
            'http://codeforces.com/api/contest.status?contestId=' +
              problems[temp].contestId +
              '&from=1&count=1000000&handle=' +
              map.get(challenge.get(msg.author.id)[0])
          );
        ...
        msg.channel.send(
          'problem for <@' +
            msg.author.id +
            '> and <@' +
            challenge.get(msg.author.id)[0] +
            '>: https://codeforces.com/contest/' +
            problems[indx].contestId +
            '/problem/' +
            problems[indx].index
        );
      ...
      case 'top':
        ...
        if (args[2] == 'point') {
          ...
          arr.forEach(function print(val, key) {
            hasil +=
              counter.toString() + '. ' + key + ' - ' + val.toString() + '\n';
            counter++;
          });
      ...
      case 'cfgraph':
        ...
        if (args.length == 2) {
          ...
          for (var i = 2; i < args.length; i++) {
            contestants[contestants.length] = args[i].substr(
              3,
              args[i].length - 4
            );
          }
        ...
        for (var i = 0; i < contestants.length; i++) {
          if (map.has(contestants[i]) == false) {
            msg.channel.send(
              contestants[i] + " haven't registered his/her handle"
            );
            return 1;
          }
          ...
          var ratings = JSON.parse(
            request(
              'GET',
              'http://codeforces.com/api/user.rating?handle=' +
                map.get(contestants[i])
            ).getBody()
          ).result;
          var temp = {
            x: [],
      ...
      case 'teamAccept':
        ...
        for (var i = 0; i < confirmed.length; i++)
          if (confirmed[i] == msg.author.id) ada = 1;
        if (ada) return 1;
        ...
        if (confirmed.length != teamA.length * 2) {
          msg.channel.send(
            'accepted!, ' + (teamA.length * 2 - confirmed.length) + ' remaining'
          );
          return 1;
        }
      ...
      case 'teamUpdate':
        ...
        for (var temp = 0; temp < prob.length; temp++) {
          for (var i = 0; i < teamA.length; i++) {
            ...
            for (var i = 0; i < submission.length; i++) {
              if (
                submission[i].problem.index == prob[temp].index &&
                submission[i].verdict == 'OK'
              ) {
                timeTeamA[temp] = Math.min(
                  timeTeamA[temp],
                  submission[i].creationTimeSeconds
                );
              }
      ...
      case 'team':
        ...
        for (var i = 2; i < args.length; i++) {
          ...
          if (map.has(cur) == false) {
            msg.channel.send(
              '<@' + cur + "> haven't registered his/her handle"
            );
            return 1;
          }
        ...
        var hasil =
          '<@' +
          msg.author.id +
          '> made a new challenge, starting rating = ' +
          rating +
          '\n**Team A**:\n';
        for (var i = 0; i < teamA.length; i++) {
          hasil += '<@' + teamA[i] + '>\n';
      ...
      case 'cfgraphhandle':
        ...
        for (var i = 0; i < contestants.length; i++) {
          ...
          const coba = request(
            'GET',
            'http://codeforces.com/api/user.rating?handle=' + contestants[i]
          );
          if (coba.statusCode >= 300) {
            console.log('handle not found');
      ...
      case 'contest':
        ...
        if (args.length == 2) {
          ...
          for (var i = 2; i < args.length; i++) {
            ...
            contestant[contestant.length] = map.get(
              args[i].substr(3, args[i].length - 4)
            );
          }
        }
        var names = '';
        ...
        for (var temp = 0; temp < contests.length; temp++) {
          ...
          if (
            averageRating < 1600 &&
            contests[temp].name.indexOf('Div. 3') < 0
          ) {
            continue;
          } else if (
            averageRating > 2100 &&
            contests[temp].name.indexOf('Div. 1') < 0
          ) {
            continue;
          } else if (
            averageRating >= 1600 &&
            averageRating <= 2100 &&
            contests[temp].name.indexOf('Div. 2') < 0
          ) {
            continue;
          }
          var can = 1;
